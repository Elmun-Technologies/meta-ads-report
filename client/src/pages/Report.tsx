...
export default function Report() {
  ...
  const topSpend = [...campaigns]
    ...
    .slice(0, 6);
  const bestCpl = campaigns
    .filter(c => c.metrics.leads >= 3 && c.metrics.cpl != null)
  ...
  const kpis: { label: string; value: string; sub: string }[] = [
    {
      ...
      sub: `${campaigns.length} kampaniya · ${pacing.dailySpend.toFixed(1)}/kun`,
    },
    {
      label: "Leads",
      value: whole(totals.leads),
      sub: `${pacing.dailyLeads.toFixed(1)} lead/kun`,
    },
    {
      label: "Cost per lead",
      value: money(totals.cpl),
      sub: `eng arzon ${money(bestCpl[0]?.metrics.cpl ?? null)}`,
    },
    {
      label: "CTR (all)",
      value: pct(totals.ctr),
      sub: `link CTR ${pct(totals.linkCtr)}`,
    },
    { label: "CPM", value: money(totals.cpm), sub: `CPC ${money(totals.cpc)}` },
    {
      label: "Reach",
      value: whole(totals.reach),
      sub: `frequency ${ratio(totals.frequency)}`,
    },
  ];
  ...
  return (
    <>
      <div className="page-head no-print">
        <div>
          <span className="kicker">Rahbariyat uchun</span>
          <h1>Rahbariyat hisoboti</h1>
          <p>
            Bitta sahifalik qisqa xulosa: asosiy ko'rsatkichlar, ko'rishdan
            murojaatgacha bo'lgan yo'l va eng muhim harakatlar. «Chop etish»
            orqali PDF qilib saqlash mumkin (Chop etish → PDF sifatida saqlash).
          </p>
        </div>
        <div className="right">
      ...
      <PageHint>
        Bu — rahbariyatga yuborish uchun tayyor bir sahifalik hisobot.{" "}
        <b>“Chop etish”</b> tugmasi brauzer chop etish oynasini ochadi, u yerda
        “PDF sifatida saqlash” ni tanlang. Bosib chiqarishda yon panel va
        tugmalar chiqarilmaydi.
      </PageHint>
      ...
      <div className="report-sheet">
        <div className="rs-head">
          <div>
            ...
            <div className="rs-meta">
              {meta.account.name} · {meta.account.externalId} ·{" "}
              {meta.period.label} · {meta.account.currency}
            </div>
          </div>
          <div className="rs-meta" style={{ textAlign: "right" }}>
            ...
            <br />
            Signal: {counts.risk} kritik · {counts.warn} diqqat · {counts.good}{" "}
            imkoniyat
          </div>
        </div>
          ...
          <tbody>
            {topSpend.map(c => (
              <tr key={c.id} style={{ cursor: "default" }}>
                ...
                <td className="num">
                  {c.metrics.leads ? whole(c.metrics.leads) : "—"}
                </td>
                <td className="num">
                  {c.metrics.cpl ? (
                    <span
                    ...
                    >
                      {money(c.metrics.cpl)} (
                      {((c.metrics.cpl! - avgCpl) / avgCpl >= 0 ? "+" : "") +
                        (((c.metrics.cpl! - avgCpl) / avgCpl) * 100).toFixed(0)}
                      %)
                    </span>
                  ) : (
                    "N/A"
          ...
          <div>
            ...
            {bestCpl.map(c => (
              <div className="rs-insight" key={c.id}>
                <b>• {c.originalName}</b> — {money(c.metrics.cpl)} CPL,{" "}
                {whole(c.metrics.leads)} lead.
              </div>
            ))}
            <div className="rs-insight">
              <b>• Prognoz:</b> joriy surat ({pacing.dailySpend.toFixed(0)}/kun)
              30 kunda ≈ {money(pacing.projected30Spend)} sarf va ≈{" "}
              {whole(pacing.projected30Leads)} lead.
              {pacing.scale && (
                <>
                  {" "}
                  «{pacing.scale.name}» ga +$100 ≈ +
                  {pacing.scale.extraLeadsPer100} lead.
                </>
              )}
            </div>
          </div>
        ...
        <div className="rs-foot">
          Metodika: CPL = Spend / Leads; lead qaytmaganlar N/A. Ma'lumotlar{" "}
          {meta.sourceLabel} dan,{" "}
          {new Date(meta.syncedAt).toLocaleString("uz-UZ")} da sinxronlangan.
          Hisobotda faqat real eksport raqamlari ishlatilgan.
          {meta.limitations.length
            ? ` Cheklovlar: ${meta.limitations.join(" ")}`
            : ""}
        </div>
      </div>
      ...
      >
        ...
        >
          «Chop etish / PDF» tugmasi brauzer chop etish oynasini ochadi → maqsad
          sifatida <b>«Save as PDF»</b> ni tanlang. Hisobot avtomatik yorug'
          rejimga o'tadi, sidebar va tugmalar chiqmaydi.
        </p>
      </Panel>
    </>
  );
