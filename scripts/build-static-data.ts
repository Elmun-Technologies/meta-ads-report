...
async function main() {
  ...
  /** Barcha platformalarni birlashtirgan "all" snapshot — server bilan bir xil
   * logika (buildUnifiedSnapshot) orqali, platforms kesimi bilan birga. */
  const allSnapshot = await buildUnifiedSnapshot();
  if (allSnapshot) {
    snapshotsByPlatform["all"] = allSnapshot;
  }
